		...
		if (interaction.options.getString('commandid') == 'all') {
			client.application?.commands.cache.forEach(command => {
				client.application?.commands.fetch(command.id)
					.then( async (command) => {
						await channel.send({embeds: [succes(`Succesfully fetched command: ${command.name}`, 'Succes', '', '', '')]})
						// further delete it like so:
						command.delete()
						await channel.send({embeds: [succes(`Succesfully deleted command: ${command.name}`, 'Succes', '', '', '')]})
					}).catch(console.error)
			})
			await channel.send({embeds: [succes('Succesfully deleted all commands', 'Succes', '', '', '')]})
		}
		else {
