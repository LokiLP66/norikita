	...
	async execute(interaction) {
		...
		if (!member.roles.cache.some(role => role.name === '❌') && !member.roles.cache.some(role => role.name === '❌❌') && !member.roles.cache.some(role => role.name === '❌❌❌')) {
			...
			await interaction.reply({
				embeds: [mod('Warned', `Warned: ${member} one time`)],
			})
		}
			...
			await interaction.reply({
				embeds: [mod('Warned', `Warned: ${member} two times`)],
			})
		}
			...
			await interaction.reply({
				embeds: [mod('Warned', `Warned: ${member} three times`)],
			})
		}
		else {
			...
			await interaction.reply({
				embeds: [mod('Muted', `Muted: ${member} for seven days because he/she/it got 3 warns`)],
			})
		}
	},
}
